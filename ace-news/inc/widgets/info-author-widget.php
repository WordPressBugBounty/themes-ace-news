	...
	class Ace_News_Author_Info_Widget extends WP_Widget {
		...
		public function __construct() {
			$ace_news_author_info_widget_ops = array(
				'classname'   => 'blog-author-section ascendoor-widget author-style-2',
				'description' => __( 'Retrive Author Information Widget', 'ace-news' ),
			);
			parent::__construct(
				'ace_news_author_info_widget',
				__( 'Ascendoor Author Info Widget', 'ace-news' ),
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'ace-news' ); ?></label>:<br />
				...
				<input type="button" class="select-img" value="<?php esc_attr_e( 'Upload', 'ace-news' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'ace-news' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'ace-news' ); ?>" style="margin-top:5px;" /><br/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'ace-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'address' ) ); ?>"><?php esc_html_e( 'Author Address:', 'ace-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'ace-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'ace-news' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'ace-news' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php sprintf( esc_html__( 'Link %s :', 'ace-news' ), $i ); ?></label>
 ...
 * @since Ace News 1.0
 **/
function ace_news_author_info_widget_image_upload_enqueue( $hook ) {
...
add_action( 'admin_enqueue_scripts', 'ace_news_author_info_widget_image_upload_enqueue' );
